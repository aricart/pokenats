...
ControlService.prototype.handleConnect = function(nc) {
  ...
  var ssid = this.nc.request(poke.POKENATS_ADMIN + '.' + poke.DISCOVER, {max:20}, (function(msg, reply, subject) {
    try {
      var m = JSON.parse(msg);
      console.log(m);
      that.updateEntry(m);
    } catch(err) {
      that.badRequest({msg: msg, reply: reply, subject: subject, err: err});
    }
  }));

  this.nc.timeout(ssid, 2000, 3, function(){
    that.emit('ready');
  });
};
...
function needsService(now, services) {
  ...
  for(var client in services) {
    if(services.hasOwnProperty(client)) {
...
ControlService.prototype.startIfNone = function(){
  ...
  types.forEach((function(serviceType) {
    if(needsService(now, this.services[serviceType])) {
      ...
      s.stdout.on('data', function(data){console.log(String(data));});
      s.stderr.on('data', function(data){console.log(String(data));});
