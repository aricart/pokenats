...
Trainer.prototype.handleUpdate = function(msg, reply, subj) {
  try {
    ...
    if(this.energy >= 100) {
      this.triggerSpawn();
      // reduce energy
      this.energy -= Math.random()*this.energy;
    }
...
Trainer.prototype.updateGeoEvents = function() {
  if(this.nc) {
    ...
    if (this.grid) {
      ...
      if (!this.geoSubIds[this.grid.index]) {
        ssid = nc.subscribe(subject, this.handleNew.bind(this));
        newSubs[this.grid.index] = ssid;
      }
    ...
    for (var n in this.geoSubIds) {
      if (this.geoSubIds.hasOwnProperty(n)) {
        if (!newSubs[n]) {
          ssid = this.geoSubIds[n];
          nc.unsubscribe(ssid);
          unsubGrids.push(n);
        }
      }
    }
    if(unsubGrids.length > 0) {
...
Trainer.prototype.enter = function() {
  // start heartbeating
  this.sendUpdateTimer = setInterval(this.sendUpdate.bind(this), this.heartbeatInterval);
  this.updateGeoEvents();
};
